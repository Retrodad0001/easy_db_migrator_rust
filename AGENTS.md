...
- **Quality check results are reported as a table** — one row per gate, covering
  ...
  | **Every code path exercised** | ❌ `DbMigrator::new` |
  | | ❌ `SystemClock::now_utc` |
  ...
  The **Every code path exercised** gate names, in the result column, every path
  no test drives — each uncovered function, match arm or error branch, listed out
  rather than counted. It takes one row per uncovered path: the gate name goes in
  the first row and the gate column is left empty on the rest, so six gaps read
  as six lines. Do not use `<br>` to break a line inside a cell — the terminal
  renderer prints the tag literally instead of breaking. A coverage gap that only
